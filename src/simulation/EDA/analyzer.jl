...
# rslt_ = [CSV.read("G:/recent/rslt $(lpad(seed, 4, '0')).csv", DataFrame) for seed = 1:10]

# tensor_rslt = reshape(rslt_[1][:, 4], 100, 2, 17);
# plot(sum(tensor_rslt[:, 1:2, 1], dims = 2), label = "2012")
# tensor_rslt = reshape(rslt_[1][:, 4 + 30], 100, 2, 17);
# plot!(sum(tensor_rslt[:, 1:2, 1], dims = 2), label = "2051")

# tensor_rslt = reshape(rslt_[1][:, 4], 100, 2, 17);
# plot(sum(tensor_rslt[:, 1:2, 3], dims = 2), label = "2012")
# tensor_rslt = reshape(rslt_[1][:, 4 + 30], 100, 2, 17);
# plot!(sum(tensor_rslt[:, 1:2, 3], dims = 2), label = "2051")

# tensor_rslt = reshape(rslt_[1][:, 4], 100, 2, 17);
# plot(sum(tensor_rslt[:, 1:2, 14], dims = 2), label = "2012")
# tensor_rslt = reshape(rslt_[1][:, 4 + 30], 100, 2, 17);
...
#     plot!(p1, 2012:2100, vec(sum(Matrix(rslt_[k][:,4:end]), dims = 1)),
...
# plot(
#   plot(2012:2100, 대구, xticks = 2020:20:2100, )
# , plot(2012:2100, 경북, xticks = 2020:20:2100, )
# )
# plot(2012:2100, 수도권 ./ (수도권 + 지방), xticks = 2020:20:2100)

# 생산인구 = vec(sum(tensor_rslt[(15:64), :, :, :], dims = 1:3))
# 부양인구 = vec(sum(tensor_rslt[Not(15:64), :, :, :], dims = 1:3))
# plot(2012:2100, 부양인구 ./ (생산인구 + 부양인구), xticks = 2020:20:2100)
...
# for k ∈ 1:10
#     tensor_rslt = reshape(rslt_[k][:, end], 100, 2, 17);
#     plot!(p2, 2012:2050, vec(sum(Matrix(rslt_[k][:,4:end]), dims = 1)),
