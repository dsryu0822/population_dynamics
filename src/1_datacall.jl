...
if isfile("data/data.csv")
    ...
    ratio_yng(popbyage) = sum(popbyage[1:15]) / sum(popbyage)
    ratio_old(popbyage) = sum(popbyage[66:end]) / sum(popbyage)
    ...
    data = outerjoin(POP, GDP, on = [:ISO3, :t])[:, [:ISO3, :t, :x, :y, :z, :dx, :dy, :dz]]
    sort!(data, [:ISO3, :t])
